...
export default function writerBlog() {
  ...
  const handleEdit = (blog) => {
    setFormData({
      ...
      blogImage: blog.blogImage?.url || null,
    });
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      setBlogs(prevBlogs =>
        prevBlogs.map(blog =>
          blog._id === editingId ? res.data.blog : blog
        )
      );
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    try {
      ...
      blogs.splice(blogs.findIndex(b => b._id === id), 1);
    } catch (err) {
  ...
  return (
    <div className="min-h-screen py-24 px-6 md:px-12 lg:px-24 bg-gradient-to-br from-black via-zinc-950/90 to-zinc-900/80 relative overflow-hidden">
      ...
      <div className="absolute inset-0">
        <div className="fixed top-1/4 left-10 w-96 h-96 bg-purple-600/30 rounded-full blur-3xl animate-pulse" />
        <div
          className="fixed bottom-1/4 right-10 w-80 h-80 bg-indigo-600/30 rounded-full blur-3xl animate-pulse"
        ...
        >
          <div className="flex flex-col lg:flex-row gap-6 items-end lg:items-center">
            <div className="relative flex-1 max-w-md group">
              <MagnifyingGlassIcon className="text-white w-6 h-6 absolute left-5 top-1/2 -translate-y-1/2  group-focus-within:text-purple-400 transition-colors" />
              <input
                type="text"
            ...
            <div className="flex items-center gap-4 flex-wrap">
              ...
              >
                ...
                <span>{showForm ? "Close Form" : "Add Blog"}</span>
              </motion.button>
            </div>
          </div>
        </motion.div>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h2 className="text-3xl font-black text-white tracking-tight">
                          {editingId ? "Edit Blog Post" : "Add New Blog"}
                        </h2>
                        <p className="text-white/80 text-lg font-medium">
                          {editingId ? "Update your existing post" : "Share your blogs with the world"}
                        </p>
                      </div>
                    </div>
                    <motion.button
